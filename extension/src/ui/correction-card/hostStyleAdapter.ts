...
export type HostSurface = {
  ...
  /** Vertical gap under the field (px). */
  gapPx: number;
};
...
/**
 * Style source for chrome (radius / border / fill).
 * Prefer the editable; if it has no visible box, use a nearby ancestor that does.
 */
export function findVisualChrome(target: HTMLElement): HTMLElement {
  ...
  while (node && depth < 6) {
    ...
    if (node.hasAttribute('data-ewa-correction-host')) {
...
export function readHostSurface(target: HTMLElement): HostSurface {
  ...
  // Background: prefer editable fill; else chrome fill; else fallback
  ...
  // Corners follow the visual input box (chrome), not a nested empty contenteditable
  let radii = resolveRadii(chrome, chromeStyles);
  if (chrome !== target && isInside(chrome, target)) {
    // Row sits inside the chrome shell — soften corners so it fits the inner surface
...
/** When the row is nested in a rounded shell, use slightly smaller matching corners. */
function insetRadii(
...
function resolvePadding(
  ...
  // Prefer text padding; fall back to chrome; enforce a readable minimum
  return {
...
/** Prefer computed corners; fall back to inline/shorthand for environments that omit them. */
...
export function applyHostSurface(el: HTMLElement, surface: HostSurface): void {
  ...
  el.style.setProperty('--ewa-bg', surface.background);
  el.style.setProperty('--ewa-bg-hover', surface.backgroundHover);
  el.style.setProperty('--ewa-bg-active', surface.backgroundActive);
  el.style.setProperty('--ewa-muted', surface.muted);
  el.style.setProperty('--ewa-fg', surface.color);
}
...
/** amount: negative darkens, positive lightens. Typical |amount| 0.03–0.08 */
