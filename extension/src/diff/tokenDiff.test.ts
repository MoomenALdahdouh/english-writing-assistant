...
describe('token diff', () => {
  ...
  it('leaves already-correct text unmarked', () => {
    const tokens = buildHighlightedTokens(
  ...
  it('handles inserts and deletes via replace collapse', () => {
    const tokens = diffTokens('She go to school', 'She went to school');
    expect(tokens.map((t) => t.value).join('')).toBe('She went to school');
  });
});
