import type { ChangeType, CorrectionChange } from '@ewa/shared';
...
/** Token-based LCS diff of the corrected view (deletes omitted). */
...
/**
 * Character LCS of one replaced word. Tie-break prefers insert so an extra
 * letter in the correction (recive → receive) marks only that letter.
 */
...
/** Build display tokens: corrected text with only changed characters marked. */
...
/**
 * History UI diff: keeps removals (wrongs) and additions (fixes) so the popup
 * can strikethrough mistakes and highlight corrections inline.
 */
export function buildHistoryDiffTokens(original: string, corrected: string): DiffToken[] {
  const a = tokenize(original);
  const b = tokenize(corrected);
  const n = a.length;
  const m = b.length;
  ...
  for (let k = 0; k < raw.length; k++) {
    ...
    const next = raw[k + 1];
    if (cur.type === 'delete' && next?.type === 'insert') {
      const changeType = classifyHistoryChange(cur.value, next.value);
      // Keep the full wrong token + full fix so edits stay obvious in history
...
function classifyHistoryChange(original: string, corrected: string): ChangeType {
  ...
  if (/^[A-Za-z]+$/.test(o) && /^[A-Za-z]+$/.test(c)) {
    ...
    // Small letter edits → spelling; larger rewrites → wording
