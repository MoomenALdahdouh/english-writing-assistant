 ...
 * Only whole-word replacements — never rewrite while mid-token.
 */
...
/** Fix completed words; leave an unfinished trailing token untouched unless it is a known typo. */
export function applyInstantSpelling(text: string): string {
  ...
  if (trailingIncomplete) {
    ...
    if (m) {
      const last = m[2]!;
      // Known typos are complete tokens — fix them even before space.
  ...
  const fixedHead = head.replace(WORD_RE, (word) => {
    ...
    if (!repl) return word;
    // Preserve simple capitalization of the first letter
