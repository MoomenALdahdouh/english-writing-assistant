...
export function extractWritingContext(text: string): string {
  ...
  let chunk =
    paragraphs.length > 1 ? (paragraphs[paragraphs.length - 1] ?? text) : text;
  ...
  if (chunk.length <= DEFAULTS.MAX_CORRECTION_CHARS) return chunk;

  const slice = chunk.slice(-DEFAULTS.MAX_CORRECTION_CHARS);
